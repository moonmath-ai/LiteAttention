...
def main():
    ...
    for i in range(warmup_iters):
        # BF16 FA3
        _ = flash_attn_func(
            q, k, v, softmax_scale=softmax_scale, causal=causal, window_size=(-1, -1)
        )
        # BF16 LiteAttention
        ...
        _ = flash_attn_func(
            ...
            causal=causal,
            window_size=(-1, -1),
            q_descale=descale_q_warmup,
            k_descale=descale_k_warmup,
            v_descale=descale_v_warmup,
        )
        # INT8 LiteAttention
    ...
    print(
        f"  Q descale range: [{descale_q.min().item():.6f}, {descale_q.max().item():.6f}]"
    )
    print(
        f"  K descale range: [{descale_k.min().item():.6f}, {descale_k.max().item():.6f}]"
    )
    print(
        f"  V descale range: [{descale_v.min().item():.6f}, {descale_v.max().item():.6f}]"
    )
    ...
    for name, output in results:
        ...
        cosine = torch.nn.functional.cosine_similarity(
            out_f32.flatten().unsqueeze(0), ref_f32.flatten().unsqueeze(0)
        ).item()
        print(
            f"{name:<25} {max_abs:<14.6e} {mean_abs:<14.6e} {rmse:<14.6e} {cosine:<12.8f}"
        )
