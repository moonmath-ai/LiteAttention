...
def test_compile_default(q, k, v):
    ...
    for i in range(3):
        ...
        assert out.shape == q.shape, f"Run {i+1}: Expected shape {q.shape}, got {out.shape}"
...
def test_module_integration(q, k, v):
    ...
    class SimpleAttnModel(nn.Module):
        def __init__(self):
            super().__init__()
            self.attn = LiteAttention(threshold=-6.0, use_int8=True, enable_skipping=True)
...
def test_changing_shapes_a_b_a():
    ...
    assert out_a1.shape == q_a.shape and out_b.shape == q_b.shape and out_a2.shape == q_a.shape
    print(f"  Pass 1 (cold, A->B->A): {elapsed1:.3f} s  (shape A={out_a1.shape}, B={out_b.shape})")
...
def main():
    ...
    print("  - Test 5: A->B->A run twice (cold then warm) to see compilation vs cached speedup")
