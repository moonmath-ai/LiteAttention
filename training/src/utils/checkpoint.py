...
def interpolate_pos_embedding(
    ...
    assert (out_seqlen % orig_emb.shape[1]) == 0, (
        "out_seqlen must be a multiple of the original sequence length"
    )
    reps = [1 for i in orig_emb.shape]
    reps[1] = out_seqlen // orig_emb.shape[1]

    if interleave:
        assert math.isqrt(orig_emb.shape[1]) ** 2 == orig_emb.shape[1], (
            "interleave only works for square lengths"
        )
        assert math.isqrt(out_seqlen) ** 2 == out_seqlen, (
            "interleave only works for square lengths"
        )
        assert math.isqrt(reps[1]) ** 2 == reps[1], (
