    ...
    def on_before_optimizer_step(
        ...
        for param_name, param in named_parameters.items():
            ...
            if param.grad is not None:
                ...
                param_grad_abs_mean = (
                    param_grad_abs.mean(dtype=torch.float32) / loss_scale
                )
                stats[f"stats/{param_name}_grad_max"] = (
                    param_grad_abs.max() / loss_scale
                )
        ...
        if trainer.loggers is not None:
            for logger in trainer.loggers:
                logger.log_metrics(
                    stats, step=trainer.fit_loop.epoch_loop._batches_that_stepped
                )
