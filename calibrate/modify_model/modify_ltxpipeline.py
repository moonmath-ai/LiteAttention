...
from .modify_ltx import LTXVideoTransformer3DModel
...
from diffusers.pipelines.ltx.pipeline_ltx_condition import (
    EXAMPLE_DOC_STRING, LTXVideoCondition,
    linear_quadratic_schedule, linear_quadratic_schedule,
    retrieve_timesteps, retrieve_latents, rescale_noise_cfg
)
...
# ---------- helpers: tuned JSON I/O ----------
def _load_tuned_array_for_stage(part: str, num_steps: int):
    """
    Load per-timestep tuned thresholds from:
        ./calibrate/models_dict/ltx/<generate_or_denoise>.json
    File content is a raw JSON list.
    Returns a list[float] of length `num_steps` (assert).
    """
    base_dir = os.path.join("calibrate", "models_dict", "ltx")
    path = os.path.join(base_dir, f"{part}.json")
    with open(path, "r") as f:
        arr = json.load(f)
    arr = [float(x) for x in arr]
    assert len(arr) == num_steps, f"Load pv_threshold: pv_threshd length {len(arr)} does not match total timesteps {len(num_steps)}."
    return arr


def _save_tuned_array_for_stage(part: str, tuned_list):
    """
    ...
    out_path = os.path.join(base_dir, f"{part}.json")
    with open(out_path, "w") as f:
        json.dump([float(x) for x in tuned_list], f)
# -------------------------------------------------------------------
...
class LTXConditionPipeline(DiffusionPipeline, FromSingleFileMixin, LTXVideoLoraLoaderMixin):
    ...
    def __call__(
        ...
        max_sequence_length: int = 256,
        # NEW: simple, explicit controls from ltx_infer.py
        mode: str = None,             # "tune" | "load" | "fixed"
        part: str = None,             # "generate" | "denoise"
        pv_l1_list: list[float] = None,      # pv_l1 list for tuning
    ):
        r"""
        ...
        # --- Decide load/tune/fixed and prep lists ---
        loaded_list = None
        if mode == "load" and part:
            loaded_list = _load_tuned_array_for_stage(part, self._num_timesteps)
        # Filled only in tuning mode (per-step)
        tuned_list: List[Optional[float]] = [None] * self._num_timesteps
        if mode == "tune":
            assert len(pv_l1_list) == num_inference_steps, f"Tune {part}: pv_l1 length {len(pv_l1_list)} does not match total timesteps {num_inference_steps}."
                        ...
                        while (hi - lo) > delta:
                            ...
                            try:
                                _ = self.transformer(
                                    hidden_states=latent_model_input,
                                    encoder_hidden_states=prompt_embeds,
                                    timestep=timestep,  # same per-step conditioning
                                    encoder_attention_mask=prompt_attention_mask,
                                    video_coords=video_coords,
                                    attention_kwargs={**base_kwargs, "tuning_pv": float(mid), "pv_l1": float(target)},
                                    return_dict=False,
                                )[0]
                                feasible_seen = True
                                best = mid
        ...
        if mode == "tune" and part:
            _save_tuned_array_for_stage(part, tuned_list)
            print(f"Tuned pv_thresholds for ltx {part}:")
            print(tuned_list)
