...
def get_tile_seq_k(head_dim):
    ...
    if head_dim not in tile_map:
        raise ValueError(
            f"Unsupported head_dim={head_dim}. Only 64, 96, 128, 192, 256 supported."
        )
...
def quantize_qk(Q, K, dtype="f32"):
    ...
    Q_q = torch.zeros(
        batch, seqlen_q, num_heads, head_dim, device=device, dtype=torch.int8
    )
    K_q = torch.zeros(
        batch, seqlen_k, num_heads, head_dim, device=device, dtype=torch.int8
    )
    ...
    q_scales = torch.zeros(
        total_bh, num_seq_tiles_q, device=device, dtype=torch.float32
    )
    k_scales = torch.zeros(
        total_bh, num_seq_tiles_k, device=device, dtype=torch.float32
    )
    ...
    func = {"f16": quant_tma.quantize_qk_f16, "bf16": quant_tma.quantize_qk_bf16}[dtype]
...
def test_correctness(
    batch, seqlen_q, seqlen_k, num_heads, head_dim, dtype="f32", name=""
):
    ...
    Q = (
        torch.randn(
            batch, seqlen_q, num_heads, head_dim, device=device, dtype=torch_dtype
        )
        * 0.1
    )
    ...
    K = (
        torch.randn(
            batch, seqlen_k, num_heads, head_dim, device=device, dtype=torch_dtype
        )
        * 0.1
    )
    ...
    # Run quantization
    Q_q, K_q, q_scales, k_scales, k_mean = quantize_qk(
        Q.contiguous(), K.contiguous(), dtype
    )
    ...
    print(
        f"  {status} {name} [{batch},{seqlen_q},{seqlen_k},{num_heads},{head_dim}] {dtype}: "
