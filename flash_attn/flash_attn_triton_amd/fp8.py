...
class FlashAttnFP8Func(torch.autograd.Function):
    ...
    def forward(
        ...
        if is_fp8(q) or is_fp8(k) or is_fp8(v):  # fp8 input and output
            ...
            ), f"You need to pass descale factors for q, k and v"
            q_fp8 = q
            k_fp8 = k
            v_fp8 = v
            ...
            assert (descale_q is None) and (descale_k is None) and (descale_v is None), (
                f"Found {q.dtype} input tensor with descale factors. In this case, we cast to fp8 and compute the descale factors. You can pass an fp8 tensor with its descale factors if desired."
            )
        ...
        out = out_fp8[
            ..., :head_size_og
        ]  # NOTE: this used to be out_padded. It might cause issue doing an empty

        # check output type
        assert out.dtype == q.dtype, (
            "Input and output type must match otherwise there will be implicit casting by autograd"
        )
    ...
    def backward(ctx, dout, *args):
        ...
        if is_fp8(dout):  # fp8 input and output
            ...
            assert descale_do is None, (
                f"Found {dout.dtype} input tensor with descale factors. In this case, we cast to fp8 and compute the descale factors. You can pass an fp8 tensor with its descale factors if desired."
            )
        ...
        dout_padded_fp8, q_fp8, k_fp8, v_fp8, out_fp8 = [
            maybe_contiguous(x) for x in (dout_padded_fp8, q_fp8, k_fp8, v_fp8, out_fp8)
        ]
        flash_attn_gpu.bwd(
            dout_padded_fp8,
            q_fp8,
...
class FlashAttnVarlenFP8Func(torch.autograd.Function):
    ...
    def forward(
        ...
        if is_fp8(q) or is_fp8(k) or is_fp8(v):  # fp8 input and output
            ...
            q_fp8, descale_q = cast_to_fp8(
                q, torch.float8_e4m3fnuz, "thd", cu_seqlens=cu_seqlens_q, max_seqlen=max_seqlen_q
            )
            k_fp8, descale_k = cast_to_fp8(
                k, torch.float8_e4m3fnuz, "thd", cu_seqlens=cu_seqlens_k, max_seqlen=max_seqlen_k
            )
            v_fp8, descale_v = cast_to_fp8(
                v, torch.float8_e4m3fnuz, "thd", cu_seqlens=cu_seqlens_k, max_seqlen=max_seqlen_k
            )
            out_fp8, descale_o = torch.zeros_like(q_fp8, dtype=torch.float32), None
    ...
    def backward(ctx, dout, *args):
        ...
        if is_fp8(dout_padded):  # fp8 input and output
            ...
            dout_padded_fp8, descale_do = cast_to_fp8(
                dout_padded,
...
class FlashAttnQKVPackedFP8Func(torch.autograd.Function):
    ...
    def backward(ctx, dout, *args):
        ...
        if is_fp8(dout):  # fp8 input and output
            ...
            dout_padded_fp8 = dout_padded
            dqkv, descale_dqkv = (
                torch.zeros(qkv_shape, device=q_fp8.device),
            ...
            dout_padded_fp8, descale_do = cast_to_fp8(dout_padded, torch.float8_e4m3fnuz, "bshd")
            dqkv, descale_dqkv = (
                torch.zeros(qkv_shape, dtype=torch.float32, device=q_fp8.device),
                None,
            )
...
class FlashAttnVarlenQKVPackedFP8Func(torch.autograd.Function):
    ...
    def forward(
        ...
        if is_fp8(q) or is_fp8(k) or is_fp8(v):  # fp8 input and output
            ...
            q_fp8, descale_q = cast_to_fp8(
                q, torch.float8_e4m3fnuz, "thd", cu_seqlens=cu_seqlens, max_seqlen=max_seqlen
            )
            k_fp8, descale_k = cast_to_fp8(
                k, torch.float8_e4m3fnuz, "thd", cu_seqlens=cu_seqlens, max_seqlen=max_seqlen
            )
            v_fp8, descale_v = cast_to_fp8(
                v, torch.float8_e4m3fnuz, "thd", cu_seqlens=cu_seqlens, max_seqlen=max_seqlen
            )
            out_fp8, descale_o = torch.zeros_like(q_fp8, dtype=torch.float32), None
