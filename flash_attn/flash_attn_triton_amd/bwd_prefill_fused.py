...
def _flash_attn_forward(
    ...
    if return_softmax or enable_dropout:
        s_dmask = torch.zeros(
            (batch, num_q_heads, max_seqlen_q, max_seqlen_k), device=q.device, dtype=torch.float32
        )
...
def _bwd_dkdvdq_inner(
    ...
    dq_ptrs_start = (
        DQ + offs_m[:, None] * stride_q_m + offs_k[None, :] * stride_q_k
...
def _bwd_kernel_dkdvdq_causal(
    ...
    for head_q_idx in range(head_k_idx * GROUP_SIZE, head_k_idx * GROUP_SIZE + GROUP_SIZE):
        ...
        adj_delta = (
            batch_idx * stride_delta_b + head_q_idx * stride_delta_h + q_start * stride_delta_m
        )
        ...
        if ENABLE_DROPOUT:
            batch_philox_offset = (
                philox_offset_base + batch_idx * stride_dropout_b + head_q_idx * stride_dropout_h
            )
            dropout_offset = (
    ...
    def forward(
        ...
        if is_grad:
            ctx.save_for_backward(
                q_fp8, k_fp8, v_fp8, out_padded, softmax_lse, descale_q, descale_k, descale_v
            )
            ctx.philox_seed = philox_seed
            ctx.philox_offset = philox_offset
            ctx.dropout_p = dropout_p
