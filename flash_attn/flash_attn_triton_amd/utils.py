...
DEBUG_TRITON = (
    os.environ.get("DEBUG_TRITON", "0").lower() in ("1", "true", "yes") and USE_TRITON_INTERPRET
)
DEBUG_TRITON_DETAIL = (
...
def generate_varlen_tensor(
    ...
    cu_seqlens = (
        torch.cat([torch.tensor([0], dtype=torch.int32, device=device), seqlens.cumsum(dim=0)])
        .to(torch.int32)
        .to(device=device)
    )
        ...
        if layout == "bshd":
            if is_fp8_dtype:
                q, descale_q = generate_bshd_tensor(
                    BATCH, N_CTX_Q, HQ, D_HEAD, dtype=dtype, device=device, DEBUG_INPUT=DEBUG_INPUT
                )
                k, descale_k = generate_bshd_tensor(
                    BATCH, N_CTX_K, HK, D_HEAD, dtype=dtype, device=device, DEBUG_INPUT=DEBUG_INPUT
                )
            ...
            if is_fp8_dtype:
                q, descale_q = generate_bhsd_tensor(
                    BATCH, HQ, N_CTX_Q, D_HEAD, dtype=dtype, device=device, DEBUG_INPUT=DEBUG_INPUT
                )
                k, descale_k = generate_bhsd_tensor(
                    BATCH, HK, N_CTX_K, D_HEAD, dtype=dtype, device=device, DEBUG_INPUT=DEBUG_INPUT
                )
...
def cast_to_fp8(
    ...
    stride_out_batch, stride_out_head, stride_out_seq, stride_out_dim = get_stride_from_layout(
        x_fp8, layout
    )
...
def get_shapes_from_layout(
    q, k, layout, cu_seqlens_q=None, cu_seqlens_k=None, max_seqlen_q=None, max_seqlen_k=None
):
...
def get_shape_and_strides_from_layout(
    ...
    return get_shape_from_layout(x, layout, cu_seqlens, max_seqlen), get_stride_from_layout(
        x, layout
    )
