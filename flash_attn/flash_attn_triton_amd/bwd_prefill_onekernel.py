...
def bwd_kernel_causal(  # grid = (tl.cdiv(max_seqlen_q // BLOCK_M2), batch, nheads_q)
    ...
    if start_n < seqlen_k:
        ...
        if delta_qk >= 0:
            ...
            if DEBUG_TRITON:
                print(f"q >= k: start_delta = delta_qk aligned to BLOCK_M = {start_delta_q_gt_k}")  # noqa: E701
        else:
        ...
        for hqid in range(hkid * GROUP_SIZE, hkid * GROUP_SIZE + GROUP_SIZE):
            ...
            if DEBUG_TRITON:
                print(f"Masked: start_n: {start_n}; start_m: {start_m}, num_steps: {num_steps}")  # noqa: E701
    ...
    if start_m < seqlen_q:
        ...
        if DEBUG_TRITON:
            print(f"end_n = start_m + BLOCK_M = {start_m} + {BLOCK_M2} = {start_m + BLOCK_M2}")  # noqa: E701
...
def attention_prefill_backward_triton_split_oneKernel_impl(
    ...
    dq_strides, dk_strides, _, do_strides = get_strides_from_layout(dq, dk, dv, do, layout)
