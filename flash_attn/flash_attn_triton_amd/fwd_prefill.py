...
def attn_fwd(
    ...
    if ENABLE_DROPOUT:
        ...
        dropout_mask_ptrs = (
            dropout_mask_offset + offs_m[:, None] * stride_sm + offs_n[None, :] * stride_sn
        )
        batch_philox_offset = (
            philox_offset_base + off_z * stride_sz + off_h_q * stride_sh
        )  # + cu_seqlens_q_start * stride_sm
        philox_ptrs = (
            batch_philox_offset + offs_m[:, None] * stride_sm + offs_n[None, :] * stride_sn
        )
    else:
    ...
    l_offset = (
        LSE + off_z * stride_lse_z + off_h_q * stride_lse_h + cu_seqlens_q_start * stride_lse_m
    )
    l_ptrs = l_offset + offs_m * stride_lse_m
    if USE_EXP2:
        RCP_LN2: tl.constexpr = 1.4426950408889634
    ...
    if IS_FP8:
        ...
        if is_fp8(o):
            ...
            assert descale_o is not None, (
                f"descale_o is None. In fp8, you need to pass a tensor for descale_o along with a tensor for the output."
            )
        else:
            FP8_OUTPUT = False
    ...
    if use_dropout or return_softmax:
        ...
        if DROPOUT_USE_PYTORCH:
            dropout_mask = create_dropout_mask(
                dropout_p, (batch, nheads_q, max_seqlens_q, max_seqlens_k), seed=philox_seed
            )
        else:
