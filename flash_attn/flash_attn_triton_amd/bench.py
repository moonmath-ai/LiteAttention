...
def get_fn_params(fn_name):
    ...
    supported_dtypes = SUPPORTED_DTYPES.get(
        fn_name, [torch.float16]
    )  # default to float16 if not found
    supported_backends = [
        backend
        for backend in SUPPORTED_BACKENDS.get(fn_name, ["triton"])
        if backend in available_backends()
    ]  # default to triton backend
                        ...
                        for d_head in d_head_values:
                            for causal in causal_values:
                                for dropout in dropout_values:
                                    ...
                                    if (
                                        estimate_memory(input_config) > 8 * 1024 * 1024 * 1024
                                    ):  # 8 GB limit
                                        continue
...
def create_benchmark_fn(flash_attn, fn_name, fn_input, mode: Literal["fwd", "bwd", "full"]):
    if DEBUG:
...
def run_benchmark(func_config: FunctionConfig, input_configs):
    ...
    print(
        f"Total time for benchmarking {fn_name} in {mode} mode with {dtype}: {elapsed_time:.2f} seconds"
    )
    ...
    if has_multiple_func_configs:
        if len(function_configs) == 2:
            ...
            if is_triton_vs_ck:
                ...
                print(
                    f"Ratio values: values > 1 mean {func1} is faster than {func2} (by that factor), values < 1 mean slower"
                )
