    ...
    def forward(self, x):
        ...
        if self.use_fp8:
            context = flash_attn_qkvpacked_fp8_func(
                qkv_packed, dropout_p=self.dropout_p, causal=self.causal
            )
        else:
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4.0, causal=True, dropout=0.1, use_fp8=False):
        ...
        self.attn = FlashAttention(
            dim, num_heads=num_heads, causal=causal, dropout=dropout, use_fp8=use_fp8
        )
...
class FlashLM(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                TransformerBlock(
                    dim, num_heads, mlp_ratio, causal=causal, dropout=dropout, use_fp8=use_fp8
                )
...
def prepare_dataset(batch_size, is_varlen=False, min_len=10, max_len=256, ratio_shorter=0.7):
    ...
    )  # join non-empty lines into a single corpus string
    tokens = corpus.split()
    ...
    print(
        f"vocab size: {vocab_size}, train samples: {len(train_dataset)}, validation samples: {len(val_dataset)}"
    )
...
def main():
    ...
    train_dataloader, val_dataloader, vocab_size = prepare_dataset(
        batch_size, max_len=max_len, is_varlen=is_varlen
    )
