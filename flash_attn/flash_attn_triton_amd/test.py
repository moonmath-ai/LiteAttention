...
from .utils import DEBUG, arch_supports_fp8, input_helper
...
def test_op_prefill_fwd_impl(
    ...
    q, k, v, do, metadata = input_helper(
        BATCH,
        HQ,
        HK,
        N_CTX_Q,
    ...
    if metadata.return_scores:
        ...
        torch.testing.assert_close(
            sd_mask_triton.to(sd_mask_ref.dtype), sd_mask_ref, atol=ATOL, rtol=RTOL
        )
...
def test_op_prefill_bwd_impl(
    ...
    dq_triton = torch.zeros_like(
        q_triton, dtype=q.dtype
    )  # NOTE: the kernel does inplace accumlation on dq so dq has to be zeros
    dk_triton = (
        torch.zeros_like(k_triton, dtype=k.dtype)
        if DEBUG_INPUT
        else torch.empty_like(k_triton, dtype=k.dtype)
    )
    dv_triton = (
        torch.zeros_like(v_triton, dtype=v.dtype)
        if DEBUG_INPUT
        else torch.empty_like(v_triton, dtype=v.dtype)
    )
...
def test_ir(
    BATCH, HQ, HK, N_CTX_Q, N_CTX_K, D_HEAD, causal, dropout_p, layout, packing, test_backward
):  # Don't run this test in parallel. It clears the cache so it doesnot work properly if run in parallel.
    torch.manual_seed(20)
    device = "cuda"
    window_size = (-1, -1)
    softcap = 0.0
    ...
    for attempt in range(max_retries):
        ...
        if ttir_files:
            ...
            if attempt < max_retries - 1:
                ...
                logging.warning(
                    f"No .ttir files found on attempt {attempt + 1}. Retrying in {retry_delay}s..."
                )
