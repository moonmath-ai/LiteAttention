...
def remap_state_dict_hf_btlm(state_dict, config):
    ...
    def key_mapping_ln(key):
        ...
        key = re.sub(
            r"^transformer.h.(\d+).ln_(1|2).(weight|bias)", r"transformer.layers.\1.norm\2.\3", key
        )
        return key
    ...
    def key_mapping_mlp(key):
        key = re.sub(
            r"^transformer.h.(\d+).mlp.c_proj.bias", r"transformer.layers.\1.mlp.fc2.bias", key
        )
        return key
    ...
    def key_mapping_attn(key):
        key = re.sub(
            r"^transformer.h.(\d+).attn.c_attn.bias", r"transformer.layers.\1.mixer.Wqkv.bias", key
        )
        key = re.sub(
