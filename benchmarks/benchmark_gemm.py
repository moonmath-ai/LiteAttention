...
for k in [
    ...
    timing = benchmark_forward(
        torch.matmul, a, b, desc=backendBLAS, verbose=verbose, repeats=repeats
    )[1]
    tflops_matmul[k] = nFLOPS_matmul / timing.mean * 1e-12
    print(
        f"[torch.utils.benchmark] {backendBLAS}, {m = }, {n = }, {k = }: {timing.mean * 1e3:.3f}ms, {tflops_matmul[k]:.1f} TFLOPS"
    )
    time.sleep(2)  # to reduce power throttling
    ms = do_bench(lambda: torch.matmul(a, b), warmup=10, rep=repeats)
    tflops_matmul1[k] = nFLOPS_matmul / ms * 1e-9
    print(
        f"[triton.test.do_bench]  {backendBLAS}, {m = }, {n = }, {k = }: {ms:.3f}ms, {tflops_matmul1[k]:.1f} TFLOPS"
    )
