...
def cudnn_spda_setup(q, k, v, causal=False, window_size_left=None):
    ...
    o_gpu = torch.empty(
        (b, nheads, seqlen_q, headdim_v), dtype=q.dtype, device=q.device
    )
        ...
        for causal in [True]:
            ...
            if cudnn is not None:
                ...
                if headdim <= 256 and dtype != torch.float8_e4m3fn:
                    ...
                    if has_backward:
                        ...
                        time_b[(causal, headdim, batch_size, seqlen), "cuDNN"] = (
                            m2b.mean
                        )
            ...
            if flash_attn_func_python is not None:
                ...
                if (
                    ...
                    print(
                        f"FAv2 Python bwd: {m1b_py.mean * 1e3:.3f}ms, {(2.5 * nFLOPS / m1b_py.mean * 1e-12):.1f} TFLOPS"
                    )
