...
def main():
    ...
    # bl = 1
    # q = torch.ones(batch_size, seqlen, num_heads, headdim, 
    #                 device=device, dtype=torch.bfloat16, requires_grad=False)
    # k = torch.ones(batch_size, bl, num_heads, headdim, 
    #                 device=device, dtype=torch.bfloat16, requires_grad=False)
    # v = torch.ones(batch_size, bl, num_heads, headdim, 
    #                 device=device, dtype=torch.bfloat16, requires_grad=False)
    ...
    # Number of iterations for timing (to get average latency)
    num_timing_iters = 20
    ...
    # Create CUDA events for high-accuracy timing
    start_event = torch.cuda.Event(enable_timing=True)
    end_event = torch.cuda.Event(enable_timing=True)
    
    # Single forward pass for output
    ...
    print(f"BF16 Reference Latency: {elapsed_time_ms:.4f} ms (avg over {num_timing_iters} iterations)")
    latency_bf16_ref = elapsed_time_ms
    ...
    start_event.record()
    for _ in range(num_timing_iters):
        _ = lite_attn(q, k, v, scale=softmax_scale)
    end_event.record()
    torch.cuda.synchronize()
    elapsed_time_ms = start_event.elapsed_time(end_event) / num_timing_iters
    ...
    print(f"BF16 LiteAttention Latency: {elapsed_time_ms:.4f} ms (avg over {num_timing_iters} iterations)")
    latency_bf16_lite = elapsed_time_ms
    ...
    print(f"FP8 (no descale) Latency: {elapsed_time_ms:.4f} ms (avg over {num_timing_iters} iterations)")
    latency_fp8_no_descale = elapsed_time_ms
    ...
    print(f"FP8 (with descale) Latency: {elapsed_time_ms:.4f} ms (avg over {num_timing_iters} iterations)")
    latency_fp8_with_descale = elapsed_time_ms
    ...
    start_event.record()
    for _ in range(num_timing_iters):
        _ = lite_attn_int8(q, k, v, scale=softmax_scale)
    end_event.record()
    torch.cuda.synchronize()
    elapsed_time_ms = start_event.elapsed_time(end_event) / num_timing_iters
    ...
    print(f"INT8 LiteAttention Latency: {elapsed_time_ms:.4f} ms (avg over {num_timing_iters} iterations)")
    latency_int8 = elapsed_time_ms
    ...
    # ============================================================================
    # Summary of all latency measurements
    # ============================================================================
    print("\n" + "="*70)
    ...
    latency_results = [
        ("BF16 FA3 (Reference)", latency_bf16_ref),
        ("BF16 LiteAttention", latency_bf16_lite),
    ...
    for name, latency in latency_results:
        speedup = latency_bf16_ref / latency if latency > 0 else 0.0
        print(f"{name:<25} {latency:<15.4f} {speedup:<15.4f}x")
