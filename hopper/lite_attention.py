...
class LiteAttention(nn.Module, ConfigurableModule):
    ...
    def _enable_capture(
        ...
        self._capture_batch_indices = batch_indices if batch_indices is not None else torch.tensor([0])
    ...
    def _maybe_capture(self, write_list, threshold, query, key, scale):
        ...
        if self._capture_timesteps is not None and current_timestep not in self._capture_timesteps:
            return
        ...
        captured_skip = write_list[batch_idx][:, head_idx].clone().to(dtype=torch.int16, device="cpu")
        ...
        if res > 0:
            ...
            for bi in batch_idx:
                head_maps = []
                for hi in head_idx:
                    q_h = query[bi, :, hi, :].unsqueeze(0).unsqueeze(0)   # [1, 1, seq_q, d]
                    k_h = key[bi, :, hi, :].unsqueeze(0).unsqueeze(0)     # [1, 1, seq_k, d]
...
class LiteAttentionRegistry(ModuleRegistry):
    ...
    def enable_capture(
        ...
        batch_t = torch.tensor(batch_indices) if batch_indices is not None else torch.tensor([0])
    ...
    def _save_capture(self, path: Path) -> None:
        ...
        for name, module in self.named_modules.items():
            ...
            batch_t = module._capture_batch_indices if module._capture_batch_indices is not None else torch.tensor([0])
            ...
            timesteps = torch.tensor([d["timestep"] for d in module._captured_data], dtype=torch.int64)
            thresholds = torch.tensor([d["threshold"] for d in module._captured_data], dtype=torch.float32)
            skip_lists = torch.stack([d["skip_list"] for d in module._captured_data])
            pct_per_head = torch.stack([d["pct_per_head"] for d in module._captured_data])
            ...
            if module._capture_attn_map_res > 0:
                mod_data["attn_maps"] = torch.stack([d["attn_map"] for d in module._captured_data])
