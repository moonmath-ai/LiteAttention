        ...
        operator()(Params const &params, char *smem_buf)
        {
            ...
            { // Consumer
                ...
                )
                {
                    ...
                    static constexpr int kNRows = !LargeHeadDimV ? 2 * ((2 * kBlockM) / (ReInt8 ? (2 * NumMmaThreads) : NumMmaThreads)) : 2;
                    flash::Softmax<kNRows, /*Max_offset=*/!Is_FP8 ? 0 : 8, Is_INT8> softmax(softmax_scale_log2, seqlen_info.seqlen_q, flash::get_reint8_thread_idx_base(thread_idx, warp_group_idx));
