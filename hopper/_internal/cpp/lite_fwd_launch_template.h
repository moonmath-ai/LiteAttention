...
#include "lite_fwd_kernel_sm90.h"
...
void run_lite_fwd(Flash_fwd_params &params, cudaStream_t stream)
{
                        ...
                        BOOL_SWITCH(params.is_skipable, Is_skipable, [&] {
                            ...
                            CLUSTER_SWITCH(cutlass::ceil_div(params.seqlen_q * (!PackGQA ? 1 : params.h / params.h_k), kBlockM) % 2 == 0, Use_cluster, [&] {
                                ...
                                if constexpr (!Is_skipable) {
                                    ...
                                    run_lite_fwd<Arch, kHeadDim, kHeadDimV, ClusterM, T, T_out, Is_causal, Is_local, Has_softcap, Varlen, PagedKVNonTMA, AppendKV && Varlen, HasQv, PackGQA, Split, V_colmajor, Is_skipable, Phase, HasMustDoList>(params, stream);
                                } else {
