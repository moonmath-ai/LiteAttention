...
mha_fwd_int8(
    ...
    Tensor cu_seqlens_q,
    Tensor cu_seqlens_k,
    std::optional<Tensor> seqused_q_,
    std::optional<Tensor> seqused_k_,
    int64_t max_seqlen_q,
    int64_t max_seqlen_k,
    std::optional<double> softmax_scale_,
    bool is_causal,
    std::optional<Tensor> q_v_,
    int64_t window_size_left,
    int64_t window_size_right,
    int64_t attention_chunk,
    double softcap,
    ...
    STD_TORCH_CHECK(!deterministic, "Deterministic mode is not supported for INT8 forward.");

    printf("mha_fwd_int8\n");
    CHECK_DEVICE(q_int8); CHECK_CONTIGUOUS(q_int8);
    CHECK_DEVICE(k_int8); CHECK_CONTIGUOUS(k_int8);
    CHECK_DEVICE(v);
    CHECK_DEVICE(q_scales); CHECK_CONTIGUOUS(q_scales);
    CHECK_DEVICE(k_scales); CHECK_CONTIGUOUS(k_scales);
    CHECK_DEVICE(cu_seqlens_q); CHECK_CONTIGUOUS(cu_seqlens_q);
    CHECK_DEVICE(cu_seqlens_k); CHECK_CONTIGUOUS(cu_seqlens_k);

    STD_TORCH_CHECK(q_int8.scalar_type() == torch::headeronly::ScalarType::Char,
                    "q_int8 must have dtype torch.int8");
    STD_TORCH_CHECK(k_int8.scalar_type() == torch::headeronly::ScalarType::Char,
                    "k_int8 must have dtype torch.int8");
    STD_TORCH_CHECK(q_scales.scalar_type() == torch::headeronly::ScalarType::Float,
                    "q_scales must have dtype torch.float32");
    STD_TORCH_CHECK(k_scales.scalar_type() == torch::headeronly::ScalarType::Float,
                    "k_scales must have dtype torch.float32");
    STD_TORCH_CHECK(cu_seqlens_q.scalar_type() == torch::headeronly::ScalarType::Int,
                    "cu_seqlens_q must have dtype torch.int32");
    STD_TORCH_CHECK(cu_seqlens_k.scalar_type() == torch::headeronly::ScalarType::Int,
                    "cu_seqlens_k must have dtype torch.int32");

    if (seqused_q_.has_value()) {
        auto seqused_q = seqused_q_.value();
        CHECK_DEVICE(seqused_q); CHECK_CONTIGUOUS(seqused_q);
        STD_TORCH_CHECK(seqused_q.scalar_type() == torch::headeronly::ScalarType::Int,
                        "seqused_q must have dtype torch.int32");
    }
    if (seqused_k_.has_value()) {
        auto seqused_k = seqused_k_.value();
        CHECK_DEVICE(seqused_k); CHECK_CONTIGUOUS(seqused_k);
        STD_TORCH_CHECK(seqused_k.scalar_type() == torch::headeronly::ScalarType::Int,
                        "seqused_k must have dtype torch.int32");
    }

    auto target_dtype = v.scalar_type();
    STD_TORCH_CHECK(target_dtype == torch::headeronly::ScalarType::BFloat16 ||
                    target_dtype == torch::headeronly::ScalarType::Half,
                    "Only BF16 or FP16 value tensors are supported for INT8 forward");

    int64_t const total_q = q_int8.size(0);
    int64_t const num_heads = q_int8.size(1);
    int64_t const head_dim = q_int8.size(2);
    STD_TORCH_CHECK(q_scales.dim() == 2, "q_scales must be rank 2");
    STD_TORCH_CHECK(q_scales.size(0) == total_q && q_scales.size(1) == num_heads,
                    "q_scales shape must match (total_q, num_heads)");

    int64_t const total_k = k_int8.size(0);
    int64_t const num_heads_k = k_int8.size(1);
    int64_t const head_dim_k = k_int8.size(2);
    STD_TORCH_CHECK(head_dim_k == head_dim,
                    "k_int8 last dimension must match q_int8");
    STD_TORCH_CHECK(k_scales.dim() == 2, "k_scales must be rank 2");
    STD_TORCH_CHECK(k_scales.size(0) == total_k && k_scales.size(1) == num_heads_k,
                    "k_scales shape must match (total_k, num_heads_k)");

    namespace stable_ops = torch::stable::ops::aten;
    auto memory_format_opt = std::optional<torch::headeronly::MemoryFormat>{};
    auto q_float = stable_ops::to_dtype::call(
        q_int8,
        torch::headeronly::ScalarType::Float,
        false,
        false,
        memory_format_opt);
    auto k_float = stable_ops::to_dtype::call(
        k_int8,
        torch::headeronly::ScalarType::Float,
        false,
        false,
    ...
    auto q_scales_expanded = stable_ops::unsqueeze::call(q_scales, -1);
    auto k_scales_expanded = stable_ops::unsqueeze::call(k_scales, -1);

    auto q_scaled = stable_ops::mul::Tensor_Tensor::call(q_float, q_scales_expanded);
    auto k_scaled = stable_ops::mul::Tensor_Tensor::call(k_float, k_scales_expanded);

    auto q_cast = stable_ops::to_dtype::call(
        q_scaled,
        target_dtype,
        false,
        false,
        memory_format_opt);
    auto k_cast = stable_ops::to_dtype::call(
        k_scaled,
        target_dtype,
        false,
        false,
    ...
    auto q_contig = stable_ops::contiguous::call(q_cast);
    auto k_contig = stable_ops::contiguous::call(k_cast);

    std::optional<Tensor> q_v_cast = std::nullopt;
    if (q_v_.has_value()) {
        auto q_v = q_v_.value();
        CHECK_DEVICE(q_v);
        if (q_v.scalar_type() != target_dtype) {
            q_v = stable_ops::to_dtype::call(q_v, target_dtype, false, false, memory_format_opt);
        }
        if (!q_v.is_contiguous()) {
            q_v = stable_ops::contiguous::call(q_v);
        }
        q_v_cast = q_v;
    }
    ...
    return mha_fwd(
        ...
        q_v_cast,
        std::nullopt,
        std::make_optional(cu_seqlens_q),
        std::make_optional(cu_seqlens_k),
        std::nullopt,
        seqused_q_,
        seqused_k_,
        std::make_optional<int64_t>(max_seqlen_q),
        std::make_optional<int64_t>(max_seqlen_k),
        std::nullopt,
        std::nullopt,
        std::nullopt,
        std::nullopt,
...
void boxed_mha_fwd_int8(
    StableIValue* stack,
    uint64_t num_args,
    uint64_t num_outputs
) {
    auto q_int8 = to<Tensor>(stack[0]);
    auto k_int8 = to<Tensor>(stack[1]);
    auto v = to<Tensor>(stack[2]);
    auto q_scales = to<Tensor>(stack[3]);
    auto k_scales = to<Tensor>(stack[4]);
    auto cu_seqlens_q = to<Tensor>(stack[5]);
    auto cu_seqlens_k = to<Tensor>(stack[6]);
    auto seqused_q = to<std::optional<Tensor>>(stack[7]);
    auto seqused_k = to<std::optional<Tensor>>(stack[8]);
    auto max_seqlen_q = to<int64_t>(stack[9]);
    auto max_seqlen_k = to<int64_t>(stack[10]);
    auto softmax_scale = to<std::optional<double>>(stack[11]);
    auto is_causal = to<bool>(stack[12]);
    auto q_v = to<std::optional<Tensor>>(stack[13]);
    auto window_size_left = to<int64_t>(stack[14]);
    auto window_size_right = to<int64_t>(stack[15]);
    auto attention_chunk = to<int64_t>(stack[16]);
    auto softcap = to<double>(stack[17]);
    auto num_splits = to<int64_t>(stack[18]);
    auto pack_gqa = to<std::optional<bool>>(stack[19]);
    auto deterministic = to<bool>(stack[20]);
    auto sm_margin = to<int64_t>(stack[21]);
    auto return_attn_probs = to<bool>(stack[22]);

    auto [out, softmax_lse, out_accum, softmax_lse_accum] = mha_fwd_int8(
        q_int8,
        k_int8,
        v,
        q_scales,
...
STABLE_TORCH_LIBRARY(flash_attn_3, m) {
    ...
    m.def("fwd_int8("
        ...
        "bool deterministic = False,"
        "int sm_margin = 0,"
        "bool return_attn_probs = False) -> (Tensor, Tensor, Tensor, Tensor)");
    m.def("bwd("
        "Tensor dout,"
        "Tensor q,"
        "Tensor k,"
...
STABLE_TORCH_LIBRARY_IMPL(flash_attn_3, CUDA, m) {
    ...
    m.impl("fwd_int8", &boxed_mha_fwd_int8);
