...
def render_skip_images(
    ...
    for mod_name, mod_data in data["modules"].items():
        ...
        skip_lists = mod_data["skip_lists"]      # [n_captured, n_batch, n_heads, qtiles, ktiles+1]
        ...
        y_positions = [b * grid_h for b in range(int(height / grid_h) + 1) if b * grid_h <= height]
        x_positions = [b * grid_w for b in range(int(width / grid_w) + 1) if b * grid_w <= width]
        ...
        for ti, t in enumerate(timesteps):
            ...
            for bi_idx, b in enumerate(batch_indices):
                ...
                for hi_idx, h in enumerate(heads):
                    ...
                    plt.imshow(attn_map.numpy(), cmap="viridis", interpolation="nearest")
                    plt.title(f"{mod_name} | t={t_val} | batch {b_val} | head {h_val} | {pct:.1f}% computed")
